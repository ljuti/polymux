  ...
  module Api
    ...
    module Transformers
      ...
      def self.contract(json)
        fetch(:symbolize_keys).call(json)
      end
      ...
      def self.quote(json)
        input = fetch(:symbolize_keys).call(json)
        input = fetch(:rename_keys).call(input, sip_timestamp: :timestamp, sequence_number: :sequence)
        input[:datetime] = Time.at(input.fetch(:timestamp) / 1_000_000_000).to_datetime if input.key?(:timestamp) && input.fetch(:timestamp)
        input
      end
      ...
      def self.trade(json)
        input = fetch(:symbolize_keys).call(json)
        input = fetch(:rename_keys).call(input, sip_timestamp: :timestamp)
        input[:datetime] = Time.at(input.fetch(:timestamp) / 1_000_000_000).to_datetime if input.key?(:timestamp) && input.fetch(:timestamp)
        input
      end
      ...
      def self.market_status(json)
        return {} unless json.instance_of?(Hash)
        input = fetch(:symbolize_keys).call(json)
        fetch(:rename_keys).call(input, afterHours: :after_hours, earlyHours: :pre_market, market: :status, indiceGroups: :indices)
      end
      ...
      def self.previous_day(json)
        input = fetch(:symbolize_keys).call(json)
        fetch(:rename_keys).call(input, T: :ticker, c: :close, o: :open, h: :high, l: :low, v: :volume, vw: :vwap, t: :timestamp)
      end
      ...
      def self.snapshot(json)
        input = fetch(:symbolize_keys).call(json)
        input = fetch(:rename_keys).call(input, day: :daily_bar)
        input.delete(:last_quote) if input.key?(:last_quote) && input[:last_quote]&.empty?
        input.delete(:last_trade) if input.key?(:last_trade) && input[:last_trade]&.empty?
        input.delete(:daily_bar) if input.key?(:daily_bar) && input[:daily_bar]&.empty?
        input
      end
      ...
      def self.ticker_details(json)
        ...
        if input.key?(:address) && input.fetch(:address).instance_of?(Hash)
          input[:address] = fetch(:symbolize_keys).call(input.fetch(:address))
        end
        input
      end
      ...
      def self.stock_trade(ticker, json)
        ...
        # Note: Don't rename timestamp fields here as they're already converted above
        renames = {
          p: :price,
          s: :size,
          x: :exchange,
        ...
        # Only rename t to timestamp if we didn't already set timestamp from another field
        renames[:t] = :timestamp unless input.key?(:timestamp)

        input = fetch(:rename_keys).call(input, renames)

        # Clean up original timestamp fields that we converted
        input.delete(:sip_timestamp) if input.key?(:sip_timestamp)
        input.delete(:participant_timestamp) if input.key?(:participant_timestamp) && timestamp_field != input[:participant_timestamp]
        input.delete(:t) if input.key?(:t) && timestamp_field == input[:t]
      ...
      def self.stock_snapshot(json)
        ...
        %i[last_quote last_trade daily_bar prev_daily_bar].each do |key|
          input.delete(key) if input.key?(key) && input[key]&.empty?
        end
      ...
      def self.stock_daily_summary(json)
        ...
        fetch(:rename_keys).call(input, renames)
      end
